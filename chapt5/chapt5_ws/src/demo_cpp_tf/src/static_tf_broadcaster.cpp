...
#include <tf2_ros/static_transform_broadcaster.h>  //静态类型坐标广播器类
...
public:
    ...
    {
        broadcaster_ = std::make_shared<tf2_ros::StaticTransformBroadcaster>(this);  //在里面创建一个发布者，用来发布/tf_static话题
        this->publish_tf();
    }
    ...
    void publish_tf()
    {
        ...
        tf2::Quaternion q;
        q.setRPY(0.0, 0.0, 60*M_PI/180);  //将弧度制欧拉角度值转换为四元数
