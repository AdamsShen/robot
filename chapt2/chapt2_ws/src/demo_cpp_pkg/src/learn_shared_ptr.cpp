...
int main() 
{
    auto p1 = std::make_shared<std::string>("This is a str.");   // std::make_shared<数据类型/类>(参数);返回值，对应类的共享指针, std::shared_ptr<std::string> p1 写成 auto
    ...
    std::cout<<"p2指向内存地址数据："<<p2->c_str()<<std::endl;  //调用成员方法  "This is a str."
    // p2.reset();
    return 0;
}